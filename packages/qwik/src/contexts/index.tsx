import { NoSerialize, Signal, Slot, component$, createContextId, noSerialize, useComputed$, useContextProvider, useSignal, useStore, useTask$ } from "@builder.io/qwik";
import { isBrowser, isServer } from "@builder.io/qwik/build"
import Clerk from "@clerk/clerk-js"
import { Resources as ClerkResoures } from "@clerk/types"
import type { ActiveSessionResource, ClientResource, InitialState, OrganizationInvitationResource, OrganizationMembershipResource, OrganizationResource, UserResource } from '@clerk/types'
...
export const ClerkContextProvider = component$((props: {
  ...
  useTask$(() => {
    const run = async () => {
      if (isBrowser) {
        clerk.client = noSerialize(new Clerk(publishableKey || import.meta.env.PUBLIC_CLERK_PUBLISHABLE_KEY))

        clerk.client?.load().then(() => {
          ...
          clerk.client?.addListener((emission: ClerkResoures) => {
            ...
            state.organization = noSerialize(emission.organization ?? undefined);
            state.session = noSerialize(emission.session ?? undefined);
            state.lastOrganizationInvitation = noSerialize(emission.lastOrganizationInvitation ?? undefined);
            state.lastOrganizationMember = noSerialize(emission.lastOrganizationMember ?? undefined);
          })
        })
      }
    }
  ...
  const derivedState = useComputed$(() => deriveState(isClerkLoaded.value, state, initialStateNoSerialized))
